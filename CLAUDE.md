...
| Google Workspace through the `gws` CLI, one skill per job | `gws-shared` first when a call is stuck (sign-in, flags, failure modes) · `gws-gmail` read the mailbox, `gws-gmail-send` send a finished mail · `gws-calendar` change an appointment, `gws-calendar-agenda` just see the day · `gws-drive` files and sharing · `gws-sheets-read` and `gws-sheets-append` | Needs the one-time Google Cloud setup from `reference/gws-cli.md` |
...
**Rule 9 — session start: the open tasks are already on screen.** `reference/scripts/tasks_hier.py` runs as a `SessionStart` hook (wired in `.claude/settings.json`) and reads the task file of **the workspace the session started in**, sorted urgent-and-important first. It prints the list and then asks you to propose exactly ONE task — the topmost that is neither already running nor waiting on someone else — with the concrete first step. Two things follow from that:

- **The output is shown, not summarised.** The hook says so itself; do not compress it into prose.
- **Before you start a task, you fetch its context**: the paths and files its context line names, the project block in `PROJECTS.md`, the latest journal entries. Never open with "where did we leave off". This is also why a task that needs prior material has to name *where that material lives* — a task nobody can resume is not finished being written.

Nothing appears before the user's first message; the CLI never writes to an empty screen. On the first contact of a session (silently, without announcement if everything is fresh):
