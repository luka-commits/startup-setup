...
**On top of that 11 bundled specialist skills**, which are not commands but operating manuals for tools: `firecrawl` (web — one skill that routes all its sub-commands itself), `notebooklm` (documents turned into audio, briefings, study guides — needs a one-off install first), `playwright-cli` (browser), `docx`, `pdf`, `powerpoint` (documents), `writing-clearly-and-concisely` (text polish), `skill-creator` (building your own commands), `supabase` and `supabase-postgres-best-practices` (databases), `managed-agents` (your own agents in the cloud). They are not called, they kick in when a task needs them — **which task leads to which tool is listed above under "Tool Routing".**
